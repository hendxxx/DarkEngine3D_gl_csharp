...
// ════════════════════════════════════════════════════════════════════
//  PBR SPLAT TERRAIN (plane) — 4-layer albedo blend on top of the
//  objectPbr material pipeline. Albedo comes from the RGBA splat map
//  (painted in the viewport brush, unit 10); layers 0..3 sample their
//  own albedo texture (units 11..14). The albedo MAP (unit 0) is kept
//  as the OPTIONAL layer-0 fallback so the paintable layers stay a
//  pure ADD-ON: with a blank splat map the surface renders EXACTLY
//  like the plain objectPbr shader (zero regression).
// ════════════════════════════════════════════════════════════════════
uniform sampler2D u_splatMap;      // unit 10 — RGBA weights (painted)
uniform sampler2D u_splatAlbedo0;  // unit 11 — layer 0 albedo (fallback: layer-0 map)
uniform sampler2D u_splatAlbedo1;  // unit 12
uniform sampler2D u_splatAlbedo2;  // unit 13
uniform sampler2D u_splatAlbedo3;  // unit 14
uniform int u_splatCount;          // 1..4 active layers (layers after that are ignored)
uniform int u_splatAny;            // 0 = pure objectPbr look (blank splat map)
uniform int u_splatLayer0IsMap;    // 1 = layer 0 has no albedo → falls back to the albedo MAP
uniform float u_splatTiling = 0.5; // world UV tiling of every splat layer (same map UV space)
uniform int u_heightLayerMode = 0;   // 1 = layers auto-assigned by ELEVATION (terrain bands)
...
uniform vec3 u_splatTint3 = vec3(1.0);
...
void main() {
    ...
    // ── SPLAT BLEND (painted multi-texture) — replaces the single albedo when
    //    the splat map carries any weight. Same UV space as the height map so
    //    displaced geometry, POM offset and the painted layers stay pixel-locked;
    //    per-layer world tiling comes from u_splatTiling × u_uvScale[5]. Weights
    //    are the straight RGBA of the splat map — exact what-you-paint colors.
    if (u_splatAny == 1) {
        vec4 w = texture(u_splatMap, uvHeight - pomOffPerMap[0]);
        // ── HEIGHT LAYERS: auto-assign weights by ELEVATION — low = layer 1, high =
        //    layer N (soft smoothstep bands). Manual brush paint then ADDS on top
        ...
        if (u_heightLayerMode == 1 && u_heightLayerCount >= 1) {
            ...
            for (int i = 0; i < 4; i++) {
                if (i >= n) break;
                float lo = float(i) / float(n), hi = float(i + 1) / float(n);
                float band = 1.0;
                if (i > 0)     band *= smoothstep(lo - f, lo + f, hn);
                if (i < n - 1) band *= 1.0 - smoothstep(hi - f, hi + f, hn);
                // An empty layer slot would render as a white/black stripe —
                // zero its weight so it never participates in the blend.
        ...
        if (u_splatCount >= 2 || u_heightLayerMode == 1) {
            ...
            if (sum > 0.001) {
                ...
                vec3 a0 = u_splatLayer0IsMap == 1
                    ...
                    : texture(u_splatAlbedo0, suv).rgb * u_splatTint0;
