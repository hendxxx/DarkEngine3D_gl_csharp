...
namespace DarkEngine3D_gl_csharp.Engine
{
    public unsafe class TerrainChunkData : IDisposable
    {
        ...
        public void Generate(int ChunkSize,int worldStartX, int worldStartZ, MapLoader mapLoader)
        {
            ...
            for (int z = 0; z < size; z++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    AddQuad(vertices, xWorld, zWorld, mapLoader);
                }
            }
        ...
        private void AddQuad(List<Vertex> vertices, float x, float z, MapLoader mapLoader)
        {
            ...
            float h00 = mapLoader.GetHeight((int)x, (int)z);          // Kiri Bawah
            float h10 = mapLoader.GetHeight((int)(x + 1), (int)z);      // Kanan Bawah
            float h01 = mapLoader.GetHeight((int)x, (int)(z + 1));      // Kiri Atas
            float h11 = mapLoader.GetHeight((int)(x + 1), (int)(z + 1));  // Kanan Atas
            ...
            {
                // Use heights from the same mapLoader so normals match the mesh heights
                n00 = CalculateNormalFromMap(mapLoader, x, z);
                n10 = CalculateNormalFromMap(mapLoader, x + 1, z);
                n01 = CalculateNormalFromMap(mapLoader, x, z + 1);
                n11 = CalculateNormalFromMap(mapLoader, x + 1, z + 1);
            }

            Vector3 color = new(0.0f, 0.0f, 0.0f); // black
            color = mapLoader.GetColor((int)x, (int)z); // Override dengan warna dari mapLoader jika tersedia
        ...
        private static Vector3 CalculateNormalFromMap(MapLoader mapLoader, float x, float z)
        {
            // sample one unit around (works for integer heightmaps)
            float hL = mapLoader.GetHeight((int)(x - 1), (int)z);
            float hR = mapLoader.GetHeight((int)(x + 1), (int)z);
            float hD = mapLoader.GetHeight((int)x, (int)(z - 1));
            float hU = mapLoader.GetHeight((int)x, (int)(z + 1));

            Vector3 normal = new Vector3(hL - hR, 2.0f, hD - hU);
        ...
        private void SetupGPUResources(Vertex[] data)
        {
            ...
            GL.BindBuffer(Const.GL_ARRAY_BUFFER, VBO); // GL_ARRAY_BUFFER
        ...
        public void Draw()
        {
            if (VAO == 0) return;
            GL.BindVertexArray(VAO);
            GL.DrawArrays(Const.GL_TRIANGLES, 0, _vertexCount); // GL_TRIANGLES
