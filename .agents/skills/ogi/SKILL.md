...
- **Camera drag-pan (fly mode)**: ORTHO (2D level) = right-drag pan (selalu allowed di edit mode, hormati InGameMouseCameraControl saat play). PERSPECTIVE (tanpa 2D map) = **right-drag = TEMPORARY FREEFLY**: selama RMB ditahan → mouse-look aktif + kursor hidden + WASD gerak; lepas RMB → kursor balik, kamera berhenti (never sets FlyMouseLook — sesi sementara track di `_rmbLookWasActive`, flag publik `Camera.RmbFreeflyActive` untuk sistem lain). MMB = pan. ✈ Fly toggle = versi sticky dari look yang sama. Semua hormati toggle Mouse Camera Control (edit: `MouseCameraControl`, play: `InGameMouseCameraControl`) — flip setting langsung berefek di viewport.
- **RMB freefly vs Player2D input**: saat `Camera.RmbFreeflyActive` (RMB ditahan di perspective), `Player2DSystem` beku-kan movement/run/jump input pemain (physics/gravity jalan terus) — kalau tidak, A/D menggerakkan player DAN kamera sekaligus. Di ortho/2D RMB tetap pan sehingga input player hidup. Flag di-clear di branch `processInput=false` supaya tidak stale saat popup/modal.
...
- **Per-Frame Reset Flag Bridge**: `ScrollCapturedByUI` + `IsOverlayVisible` WAJIB di-reset SETIAP frame DI LUAR gate `Children.Count > 0` di ViewportPanel — scene kosong (0 children) tidak pernah mengeksekusi reset di dalam gate, nilai stale `true` dari scene UI sebelumnya membekukan kamera editor permanen (`SetCameraFlyMode(processInput: false)` → WASD/RMB mati total) dengan gejala "input viewport mendadak mati". Reset flags sebelum gate, draw path yang men-set kembali.
...
## Day/Night Lighting Rules (Moonlight):
- **SATU sumber kebenaran brightness malam**: `Lights.cs` — `nightBrightness` (0.45) × `nightLight` (0.05,0.07,0.14). Setelah SEMUA override (procedural + `LightColorOverride` + `SunBrightness`), `Update()` mem-fade `lightColor` → moon tint via `finalNight = Smoothstep01(0.15, 0, sunDir.Y)`. Tanpa ini, Light marker menimpa blend siang/malam dan malam tetap serumal siang (langit gelap, terrain terang).
- **JANGAN brighten-kan malam di shader**: fallback `moonColor` lama di fragment_shader.glsl (0.08,0.12,0.25 ketika `length(lightColor)<0.1`) dan (0.25,0.30,0.45) di gltf/impostor MENGganti warna malam CPU dengan nilai 2-3× lebih terang — malam jadi seperti siang mendung. Shader kini pass-through `lightColor` (guard tint = nilai gelap yang sama dengan CPU).
...
## PBR Primitives (Box/Sphere/Plane) Rules:
- **Runtime type-agnostic**: gate render PBR di `EditorObject.Draw` HANYA `HasPbrMaterial && Shader.GetObjectPbrShaderProgram() != 0` — Plane lewat `DrawPbrPrimitive` yang sama dengan Box/Sphere (7 map: albedo/normal/metallic/roughness/ao/height/emission, unit 0-6, missing map = white texel + useX=0, shader `objectPbr_fragment.glsl` + `vertex_shader.glsl`). JANGAN tambahkan gate `PrimitiveType is Box or Sphere` di jalur render.
- **Artifact plane satu-quad (DUA fix wajib, sudah terpasang)**: plane = 1 quad CCW dari atas → (1) culled/hilang saat kamera dari bawah; (2) dengan culling scene None, sisi bawah ke-shading normal ke atas = relief normal-map terbalik + gelap. Fix: `DrawPbrPrimitive` disable `GL_CULL_FACE` khusus Plane (restore via `GL.IsEnabled` — hormati scene culling None) + shader `if (!gl_FrontFacing) norm = -norm;` SEBELUM slope/TBN — flip norm ikut membalik basis T/B (derivatif cross terhadap norm) jadi dua sisi shade benar. Box/Sphere tidak berubah (closed mesh tak pernah lihat backface).
