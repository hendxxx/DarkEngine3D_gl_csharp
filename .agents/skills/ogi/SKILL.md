...
- **Runtime**: `DialogueSystem` (static, `Engine/Visual/DialogueSystem.cs`) — conversation state machine (typewriter, choices 1-9/↑↓/E, auto-advance, close-fade) + follow-bubbles. Semua drawing lewat HUD queue (`DrawBox`/`DrawText`/`DrawImage`) → identik di in-game dan IDE preview. `Tick()` dipanggil dari GameScene render pass (sebelum `_hud.Flush()`).
